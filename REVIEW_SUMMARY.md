...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Group 1**<br>Analytical Deblur (`-D analytical`) | **`origin/arena/019fc1ba`** | True full-image gradient narrowing (`method == 3`), normal tracing to plateau colors `P0, P1`, inverted `-g` semantics, 0 hull violations. | **RECOMMENDED FOR MASTER (PUSH CORE MATH `20ad551`)** | **Push/cherry-pick commit `20ad551`**. True winner for analytical deblur math; adds unit tests (`test_analytic_deblur.py`). Pair with `019fbfb9` for comparison sheets. | [`review_019fc1ba-celup-lab.md`](./review_019fc1ba-celup-lab.md) |
...
- **Why `019fc1ba` wins on mathematics (The True Analytical Engine)**:
  - **`019fc1ba` (`20ad551`)**: Implements genuine full-image gradient narrowing (`-D analytic`). It computes a 4D structure-tensor normal per pixel, traces each transition globally along `+-normal` across its entire width (up to `maxR=16` steps) to saturated plateau colors `P0` and `P1`, and applies inverted steepness narrowing (`alpha = (K-1)/K`) with **zero hull violations** by construction. It also adds the automated test suite `tests/test_analytic_deblur.py`.
  - **Why `019fbf78` created "another `autoblurcompress`"**: As noted in user feedback, `019fbf78` attempted analytical deblur using a local 2x2 bilinear projection and local S-curve mapping (`phi1(k * z0)`). Because it only operated on local neighborhood pixels rather than tracing transitions globally to their real plateaus `P0, P1`, its local unsharp projection on existing blurred ramps compressed gradients in a manner indistinguishable from `autoblurcompress`.
  - **Why `019fbfb9` is essential for visual review**: While `019fc1ba` solved the math, it omitted comparison sheet generation—meaning its output was invisible during visual review. `019fbfb9` provides the exact missing script upgrade: auto-splitting oversized WebP comparison sheets (`>16383px`) and adding analytical mode to `make_lab_comparison_sheets.py`.
- **Master Strategy**: Push/cherry-pick **`019fc1ba`** (`20ad551`) for the core analytical engine and unit tests, paired with **`019fbfb9`** (`1293c23`) for comparison sheet auto-splitting. Reject `019fbf78`'s C implementation.

### Group 2: Autodeblur Core Evolution & Diagnostics (`v4.9.3` – `v4.9.9`, `-D` autodeblur)
- **Branches Evaluated**: `019fbef6-celup-lab` vs `019fba18-celup-lab` vs `019fbcda-celup-lab`
...
# Step 6: Cherry-Pick True Analytical Deblur Math & Sheet Auto-Split (Group 1)
git cherry-pick 1293c23  # make_lab_comparison_sheets.py auto-split for >16383px WebP sheets (from 019fbfb9)
git cherry-pick 20ad551  # true full-image gradient narrowing (-D analytic), 0 hull violations & tests (from 019fc1ba)
...
4. [`review_019fbef6-celup-lab.md`](./review_019fbef6-celup-lab.md) — Polar Artifact Diagnosis, `-T` Texture Gain & Head-to-Head Comparison
5. [`review_019fba18-celup-lab.md`](./review_019fba18-celup-lab.md) — Autodeblur v4.9.4–v4.9.9, Skirt Transport & BW Grey Test Analysis
