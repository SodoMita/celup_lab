...
## v4.9.9: deblur to (nearly) zero grey on step-class sources + the pole fix

User directive: *"make autodeblur able to deblur to 0 grey on a BW
image; in most cases the highest caps of values can be removed; the
artifacts on the XY poles of circles where additional blur appears are
still there since 4.8; autodeblur is a deblur algorithm."*  All four
points measured and answered; the limits reached are analytic, not
tuned:

**Caps removed (the grey test, second round).**  The residual grey had
four classes, each killed by deleting a conservative cap, not by
adding one:

1. *Washed stroke cores* (the eye rings held 55% of the grey): the
   snap used to stop at the washed LOCAL hull floor (remnant vv ~ .084
   linear ~ lum 80 measured) because the endpoint extension was
   proportional to local/global span.  On step-class sources every
   map-admissible step is taken to connect the source-global extremes;
   the floor remnant is gone (full extension, still strictly inside
   proven colours: the source's own extremes).
2. *The 1px AA column* (the map's m = .5 fixpoint, 68% of the rest):
   the "true-AA corridor" was keeping every contour's mid column
   soft.  That information (sub-pixel width) only EXISTS in continuous
   sources; on quantized ones the column is quantizer residue.  The
   corridor now stays only for continuous content; step-class columns
   are assigned by linear hull fraction (footprint-majority coverage:
   m < .5 belongs to the dark side of the pixel's own proven hull).
3. *Partial payment*: payment is binary (full when the evidence gate
   fires, nothing when it does not); the .55-cap used to leave 30-50%
   of the proven deblur behind exactly on the washed pixels.
4. *Evidence-dead wash columns*: fit trust w is ~0 in deep wash BY
   CONSTRUCTION (the wash is what the erf cannot fit; measured w = 0
   at 60% of the residual).  Admission takes the max of fit trust and
   flank-pair proof, plus a straggler channel: near-full model-vs-
   colour mismatch with a hull spanning the global step (quantized
   only) -- the wash with no witness left.
   Frame safety comes from the TARGET, never the gate: the map pays
   toward the pixel's own plateau along the colour diagonal reached
   from its own colour, so a frame-flipped fit (the pass-1 u-reversal)
   changes which side the FIT claims, never which side the colour is
   on.  (The alternatives were measured catastrophically: a positional
   mu tie-break doubled the eye ring into salt-pepper hash; a nu-side
   plateau join put white specks inside ink, halo 107.)

**Completion premise**: the wash the map removes is generated by the
base blur, so the step-class completion fires only where blur was
actually assumed (sref > 1.2) -- a -r .5 run keeps the source
staircase verbatim (the crisp probe stays detectable), -r 2.3/6 get
the completion.

**The XY-pole artifact (present since v4.8).**  The evidence-known
slots get a separable axis-aligned box blur to fill trust holes.  At
a contour's XY poles one box axis coincides with the contour NORMAL,
so the box mixed the inner flank's evidence with the outer flank's
(opposite deltas: diluted fits, per-pixel nu flips -- measured as
extra wash exactly at the poles, then read by the snap as phantom
plateaus = notches/neon dashes).  The blur now marches along the
per-pixel tangent (bilinear taps, tent kernel, sign-free by symmetry)
-- the same consensus idea, one rule for every angle, nothing
pole-specific.

Scoreboard (smiley ROI, NN: halo 1.9, ink .949, grey 0.02%):

| metric | v4.9.5 | v4.9.7 | v4.9.8 | v4.9.9 |
|---|---|---|---|---|
| r6 grey    | 17.69% | 17.61% | 4.99%  | **1.62%** |
| r6 halo    | 17.37  | 16.25  | 8.99   | **2.91**  |
| r6 ink     | .949   | .951   | .957   | **.964**  |
| r6 darkmean| 31.6   | 30.4   | 11.4   | **10.1**  |
| r2.3 grey  | 12.71% | 12.22% | 2.96%  | **1.13%** |
| r2.3 halo  | 4.75   | 4.17   | 0.49   | **0.00**  |
| r2.3 ink   | .969   | .970   | .976   | **.988**  |
| r2.3 darkmn| 20.6   | 19.2   | 7.3    | **4.3**   |

Gates: check_corners PASS, check_stairs PASS (ship2x jump95 .096,
probe-crisp still flags .335 as designed), test_scales PASS.  miya 4x:
contour crisping only, no posterization/ringing (qconf = 0 keeps every
cap active there).  Env knobs: CELUP_NOZ, CELUP_ZG (base gain 2.2),
CELUP_ZBOOST (quantized multiplier, default 4), CELUP_ZAA (AA-corridor
half-width; continuous sources only), CELUP_RADA (evidence radii
override), CELUP_ZDBG.  Known remainder: the pole seam hs narrowed to
a sub-pixel residual dash in r6 at -r 6 (the wash there out-spans the
probe reach); r2.3 is visually NN-identical.
