...
static void print_help(const char *argv0) {
  printf(
      ...
      "                            per edge\n"
      "  -D, --deblur-method M     autodeblur method auto|remap|push|analytic\n"
      ...
      "                            analytic = produce a FULL-IMAGE gradient\n"
      "                            field, narrow every transition (push its\n"
      "                            two plateau colours toward each other) and\n"
      "                            sample from it; no per-pixel 2x2/6x6 fit.\n"
      "                            With analytic, -g is INVERTED (see below).\n"
      ...
      "                            above 8 stays artifact-safe.  For\n"
      "                            -D analytic the meaning is INVERTED:\n"
      "                            1 = maximum deblur (collapse each\n"
      "                            gradient to one point = quantize the two\n"
      "                            plateau colours), larger K = LESS deblur\n"
      "                            (retention alpha = (K-1)/K), infinity =\n"
      "                            identity; 0 still means auto.\n"
      "\n"
...
int main(int ac, char **av) {
  ...
  for (int i = 4; i < ac;) {
    ...
    if (!strcmp(av[i], "--mode") || !strcmp(av[i], "-m")) {
      ...
      if (!strcmp(av[i + 1], "auto"))
        ...
        deblur_method = 2;
      else if (!strcmp(av[i + 1], "analytic") ||
               !strcmp(av[i + 1], "analytical") ||
               !strcmp(av[i + 1], "analyse"))
        deblur_method = 3;
      else {
  ...
  if (!strcmp(mode, "adaptive")) {
    ...
    if (!strcmp(mode, "autodeblur")) {
      printf(", method=%s",
             last_deblur_method == 3 ? "analytic"
             : last_deblur_method == 2 ? "push"
                                       : "remap");
      if (last_deblur_method == 3) {
        printf(", K=%.3f%s (analytic: 1=max/quantize, higher=less)",
               last_deblur_k, deblur_steepness > 0.f ? "(manual)" : "");
      } else if (last_deblur_k > 0.f)
